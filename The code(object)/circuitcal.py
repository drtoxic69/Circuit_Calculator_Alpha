    ...
    def mixedCalculation(self):
        if self.connected.lower() == 'm':
            ...
            for sets in range(sets_of_resistors):
                values_of_resistors[f'top{sets+1}'] = list(map(float, input(f'Enter the value of resistor(s) in top of set {    sets+1}: ').split()))
                values_of_resistors[f'top{sets+1}'] = sum(values_of_resistors[f'top{sets+1}']
                values_of_resistors[f'bottom{sets+1}'] = list(map(float, input(f'Enter the value of resistor(s) in bottom in set {sets+1}: ').split()))
                print()
                # figure out a way to add the series values
